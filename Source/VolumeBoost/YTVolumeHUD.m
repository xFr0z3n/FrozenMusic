...
// FrozenMusic > YTM volume boost look: YouTube Music's grey (black with the
// OLED theme) instead of the blue blur panel
static BOOL YTVolumeHUDPref(NSString *key) {
  NSDictionary *prefs =
      [[NSUserDefaults standardUserDefaults] dictionaryForKey:@"YTMUltimate"];
  return [prefs[key] boolValue];
}

static BOOL YTVolumeHUDUsesYTMLook(void) {
  return YTVolumeHUDPref(@"frozenVolumeLook");
}

static BOOL YTVolumeHUDUsesOLED(void) {
  return YTVolumeHUDPref(@"YTMUltimateIsEnabled") &&
         YTVolumeHUDPref(@"oledTheme");
}

// Small white dot like the thumb of YTM's seek bar
static UIImage *YTVolumeHUDThumbImage(void) {
  static UIImage *thumb = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    UIGraphicsImageRenderer *renderer =
        [[UIGraphicsImageRenderer alloc] initWithSize:CGSizeMake(14.0f, 14.0f)];
    thumb = [renderer imageWithActions:^(UIGraphicsImageRendererContext *context) {
      (void)context;
      [[UIColor whiteColor] setFill];
      [[UIBezierPath bezierPathWithOvalInRect:CGRectMake(0.0f, 0.0f, 14.0f, 14.0f)] fill];
    }];
  });
  return thumb;
}
...
@property(nonatomic, assign) BOOL ytmLook;
...
- (void)applyLook {
  BOOL ytm = YTVolumeHUDUsesYTMLook();
  self.ytmLook = ytm;
  if (ytm) {
    BOOL oled = YTVolumeHUDUsesOLED();
    self.backgroundView.effect = nil;
    self.backgroundView.backgroundColor =
        oled ? [UIColor blackColor]
             : [UIColor colorWithRed:33 / 255.0 green:33 / 255.0 blue:33 / 255.0 alpha:1.0];
    self.backgroundView.layer.borderColor =
        [UIColor colorWithWhite:1.0f alpha:oled ? 0.16f : 0.06f].CGColor;
    self.slider.minimumTrackTintColor = [UIColor whiteColor];
    self.slider.maximumTrackTintColor = [UIColor colorWithWhite:1.0f alpha:0.3f];
    [self.slider setThumbImage:YTVolumeHUDThumbImage() forState:UIControlStateNormal];
    [self.slider setThumbImage:YTVolumeHUDThumbImage() forState:UIControlStateHighlighted];
  } else {
    ...
    self.backgroundView.layer.borderColor =
        [UIColor colorWithWhite:1.0f alpha:0.14f].CGColor;
    self.slider.minimumTrackTintColor = [UIColor systemBlueColor];
    self.slider.maximumTrackTintColor = [UIColor colorWithWhite:1.0f alpha:0.18f];
    [self.slider setThumbImage:nil forState:UIControlStateNormal];
    [self.slider setThumbImage:nil forState:UIControlStateHighlighted];
  }
}
...
- (void)setExpandedVisuals:(BOOL)expanded {
  ...
  self.percentLabel.textColor =
      expanded && !self.ytmLook ? [UIColor systemBlueColor] : [UIColor whiteColor];
